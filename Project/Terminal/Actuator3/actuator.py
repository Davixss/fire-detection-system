...
# Device config
DEVICE_ID = 'A' + str(random.randint(10000, 99999))   
...
ADDRESS = "4 Via Pietro Caristi, Messina, Italia"
...
def readMessages(client, userdata, message):
    ...
    try:
        if 'message_type' in message:
            # -- If device receives an ACK from the controller
            ...
            # -- If device receives a confirmation of connection from the controller
            if message['message_type'] == 'CONNECTION_OK':
                if message.get('device_id') == DEVICE_ID:
                    deviceIsConnected = True
                    Functions.printLine("GREEN", f">> Device {DEVICE_ID} is now connected")

            # -- If device receives a PING from the controller, send an ACK of my presence
            if message['message_type'] == 'PING':
                if controllerIsConnected and deviceIsConnected:
                    ...
                    mqttBroker.sendMessage('PING_ACK', response)

            # -- If device receives an ACTUATOR_ACTIVATED message
            if message['message_type'] == 'ACTUATOR_ACTIVATED':
                if message['id_device'] == DEVICE_ID:
                    coord_lat = message['coord_lat']
                    coord_lon = message['coord_lon']
                    Functions.printLine("YELLOW", f">> Event E1 triggered from [lat: {coord_lat}, lon: {coord_lon}]")
